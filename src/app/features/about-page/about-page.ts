...
export class AboutPage implements AfterViewInit, OnInit{
  ...
  content = {
    ...
    kinetics: {
      ...
      commitment: {
        ...
        description:
          "At Hair Kinetics™, every service is guided by a single goal: to deliver precision-based, natural-looking results that evolve seamlessly with each client's lifestyle. Each cut, treatment, and style is approached as a layered design, technically sound, sustainable, and personalized to the individual.",
      },
    },
    artist: {
  ...
  next() {
    this.activeIndex = (this.activeIndex + 1) % this.content.team.members.length;
    this.updateRail();
  }

  prev() {
    this.activeIndex =
      (this.activeIndex - 1 + this.content.team.members.length) % this.content.team.members.length;
    this.updateRail();
  }
  ...
  private updateRail() {
    ...
    // Move rail so activeCenter aligns with viewportCenter
